...
dataset = sbdr.XORDataset(
    seed=model_config["model"]["seed"],
)
...
outs = forward_eval_jitted(state["variables"], xs)


for k in outs.keys():
    print(f"\n{k}:{outs[k]}")

# unit-coactivation matrix
co_act = outs["z"][..., None, :] * outs["z"][..., :, None]
co_act = co_act.reshape((-1, outs["z"].shape[-1], outs["z"].shape[-1]))
co_act = co_act.mean(axis=0)
print("Unit coactivation matrix:")
print(co_act)
