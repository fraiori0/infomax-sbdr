...
"""---------------------"""
""" Covariance matrix of output units """
"""---------------------"""

fig_dict = {
    k : {kk: go.Figure() for kk in models[k].keys()} for k in models.keys()
}
...
# Update the layout of each figure in fig_dict
for k in models.keys():
    for kk in models[k].keys():
        fig_dict[k][kk].update_layout(
            title_text=models[k][kk]["name"],
            **fig_layout_single,
            template="plotly_white",
        )
