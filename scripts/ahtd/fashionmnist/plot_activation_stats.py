...
models = {
    "fashionmnist": {
        # "1": {"name": "1 layer", "chkp": 10, "color": "#d62728", "dash": "dash", "symbol": "x"},
        # "2": {"name": "2 layers", "chkp": 10, "color": "#D8A303", "dash": "dash", "symbol": "x"},
        # "3": {"name": "2 layers notd", "chkp": 10, "color": "#59956c", "dash": "dash", "symbol": "x"},
        # "3": {"name": "2 layers notd", "chkp": 17, "color": "#59956c", "dash": "dash", "symbol": "x"},
        # "4": {"name": "2 layers notd", "chkp": 28, "color": "#59956c", "dash": "dash", "symbol": "x"},
        "5": {"name": "2 layers pred", "chkp": 10, "color": "#59956c", "dash": "dash", "symbol": "x"},
    },
}
...
N_K = [N_OUT_UNITS, int(0.15*N_OUT_UNITS),] # int(0.05*N_OUT_UNITS),]
...
for n_top_k in tqdm(N_K):
    ...
    for k in tqdm(models.keys(), total=len(models.keys()), leave=False):
        for kk in tqdm(models[k].keys(), total=len(models[k].keys()), leave=False):
            ...
            acc_train = classifier_model.score(zs, labels_categorical)
            acc_val = classifier_model.score(zs_val, labels_categorical_val)
            ...
            print(f"{k} - {kk} - {n_top_k}\t{acc_train:.4f}\t{acc_val:.4f}")
