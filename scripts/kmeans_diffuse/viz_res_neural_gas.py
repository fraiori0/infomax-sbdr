...
"""Train a residual model form the residual of a previous neural gas model."""
...
with open(os.path.join(res_model_folder, "config.toml"), "r") as f:
    res_model_config = toml.load(f)
...
history_test = {
    "enc": {k : [] for k in outs["enc"].keys()},
    "res": {k : [] for k in outs["res"].keys()},
}
history_test["x"] = []
history_test["x_res"] = []
...
for xs, labels in tqdm(dataloader_test, total=len(dataloader_test)):
    ...
    for k in outs["enc"].keys():
        history_test["enc"][k].append(outs["enc"][k])
    for k in outs["res"].keys():
...
for k in history_test.keys():
    if isinstance(history_test[k], dict):
        for k2 in history_test[k].keys():
            history_test[k][k2] = np.concatenate(history_test[k][k2], axis=0)
            print(f"\t{k}.{k2} : {history_test[k][k2].shape}")
    else:
        history_test[k] = np.concatenate(history_test[k], axis=0)
        print(f"\t{k} : {history_test[k].shape}")


pprint(get_shapes(history_test))
