...
_, dparams = update_params(variables, x=xs, out=outs, t=t)
...
def t_fn(epoch_idx, batch_idx):
    ...
    n_epochs = model_config["training"]["epochs"]
    ...
    for epoch in tqdm(range(model_config["training"]["epochs"])):
        ...
        for batch_idx, (xs, labels) in tqdm(
            ...
            for metric, value in metrics.items():
                writer.add_scalar(
                    "batch/" + metric + "/train", value.item(), global_step=LAST_STEP
                )
            ...
            if batch_idx % model_config["validation"]["eval_interval"] == 0:
                ...
                for metric, value in metrics_val.items():
                    writer.add_scalar(
                        "batch/" + metric + "/val", value.item(), global_step=LAST_STEP
                    )
