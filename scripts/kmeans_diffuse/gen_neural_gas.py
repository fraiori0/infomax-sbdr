...
import infomax_sbdr.analytic_score_diffusion as an_diff
...
# norm_val = norm_val/norm_val.min(-1, keepdims=True)
# diff = diff/norm_val[..., None]
# print(diff)
# # This is the distance from each centroid/data point but only on each centroid's affected features
...
# print(an_diff.logsumexp_masked(log_likelihoods, mask))
# print()
...
# log_weights = log_likelihoods[..., None] - an_diff.logsumexp_masked(log_likelihoods, mask)[..., None, :]
# print(np.sum(log_weights, axis=-2))
# print(np.sum(log_weights, axis=-2).shape)
# log_weights = np.exp(log_weights)

# print(log_weights)
# print(log_weights.shape)

# ws = an_diff.compute_posterior_weights_masked(phi, ks, mask, alpha_bar_t)
# print(ws)
# print(ws.shape)

# ws = an_diff.compute_posterior_weights(phi, ks, alpha_bar_t)
# print(ws)
# print(ws.shape)
...
mask = state["variables"]["params"]["mask"].copy()
t0 = 0.1
phi_T = jax.random.truncated_normal(key, -1.0, 1.0, (3, ks.shape[-1]))
alpha_bar_t = an_diff.cosine_schedule(t0)
...
weights = an_diff.compute_posterior_weights_masked(phi_T, ks, mask, alpha_bar_t)
print(f"\tweights shape: {weights.shape}")
score = an_diff.ideal_score_masked(phi_T, ks, mask, alpha_bar_t)
...
phi_ts, aux = an_diff.diffuse_with_subsets_masked(
    key,
    phi_T,
    ks,
    mask,
...
# for each sample in the batch dimension (second dimension of phi_ts)
for batch_i in range(phi_ts.shape[1]):
    ...
    fig.add_trace(
        go.Heatmap(
            z=phi_ts[0, batch_i].reshape(28, 28)[::-1],
            colorscale="plasma",
            showscale=False,
            colorbar=dict(
                title="Value",
    ...
    for i in range(phi_ts.shape[0]):
        step = dict(
            method="update",
            args=[{"z": phi_ts[i, batch_i].reshape(28, 28)[::-1]}, {"title": f"Time step {i}"}],
            label=f"Time step {i}",
        )
        steps.append(step)
