...
def classification_loss(logits, labels):
    ...
    # Average/Sum logits over time axis
...
def cross_infonce(y_1, y_2):
    ...
    p_avg = (y_1 * y_1_avg + y_2 * y_2_avg).sum(-1) + model_config["training"]["loss"]["eps"]
    loss_val = -np.log(p_ii / p_avg).mean()
...
for l_idx in range(len(outs)):
    ...
    # e = outs[l_idx]["e"]
    ov = outs[l_idx]["o"]
    q_loss_val, q_aux = encoder_infonce(q)
    k_loss_val, k_aux = encoder_infonce(k)
    ov_loss_val, ov_aux = encoder_infonce(ov)
    print(f"\tLosses: layer {l_idx}: q: {q_loss_val}, k: {k_loss_val}, o: {ov_loss_val}")
...
def loss_fn_gen(state, batch):
    def loss_fn(params):
        ...
        for l_idx in range(len(outs)):
            ...
            q_loss_val, q_aux = encoder_infonce(q)
            k_loss_val, k_aux = encoder_infonce(k)
            # ov_loss_val, ov_aux = encoder_infonce(ov, eps=1.0)
            # e_loss_val = (e**2).mean()
            ...
            for k, v in k_aux.items():
                metrics.update({f"aux_losses_{l_idx}/k_{k}": v})
            for k, v in q_aux.items():
                metrics.update({f"aux_losses_{l_idx}/q_{k}": v})
            # metrics.update({f"aux_losses_{l_idx}/e_loss": e_loss_val})
