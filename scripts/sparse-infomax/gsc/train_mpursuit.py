...
CACHE_DIR:   str | None = "./cache/gsc_positive"     # ← set to None to skip disk caching

transform = sbdr.SpecAugmentTransform(
    **model_config["dataset"]["transform"]["specaugment"]["kwargs"],
)

# NON NEGATIVE: mel kwargs are different from standard
mel_kwargs = {
    "f_min" : 50,
    "n_mels": 80,
}
...
dataset_train = sbdr.GSCDataset(
    ...
    precompute  = True,
    augment     = transform,
    log_mel_kwargs=mel_kwargs,
    max_samples = MAX_SAMPLES,
    cache_dir   = CACHE_DIR,
    # NOTE: important, non-negative for matching pursuit with non-negative dict
    non_negative=True,
)
...
def classification_loss(logits, labels, gates=None):
    ...
    # compute accuracy
    aux = {
        "acc": (labels == logits.argmax(-1)).mean(),
    }
    return loss.mean(), aux

def cross_infonce(y_1, y_2):
...
def loo_infonce(a, labels, eps=None):
    ...
    a_loo_avg = sbdr.loo_label_means(
        activations=a,
        labels=labels,
        n_labels=model_config["model"]["kwargs"]["out_labels"],
...
def mse_loss(target, pred):
    loss_val = ((target - pred)**2).sum(-1).mean()
    aux = {
        "mse" : loss_val,
    }
    return loss_val, aux
...
# NON NEGATIVE important
# Create a mask selective of only the dictionary in each layer, we will use it to keep 
# it non-negative
params_mask = model.get_params_dict_mask(variables["params"])
...
def loss_fn_gen(state, batch):
    def loss_fn(params):
        ...
        for l_idx in range(len(outs)):
            ...
            for k, v in x_aux.items():
                metrics.update({f"aux_losses_{l_idx}/x_{k}": v})           
