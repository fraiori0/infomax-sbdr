...
def eval_step(state, batch):
    ...
    loss_fn = loss_fn_gen(state, batch, eval=True)
    # compute loss
...
"""------------------"""
""" Record accuracy on whole validation set """
"""------------------"""

# pass through validation set
# and print average stats over all samples

acc = []

for (xs, labels) in tqdm(dataloader_val, total=len(dataloader_val)):
    ...
    acc.append(metrics["class/acc"])

acc = np.array(acc)
print(f"Accuracy on whole validation set: {np.mean(acc)}")