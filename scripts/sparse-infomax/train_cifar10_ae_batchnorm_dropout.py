...
((zs, neg_pmi), xs_rec), mutable_updates = forward_jitted(variables, xs, key)
((zs, neg_pmi), xs_rec), _ = forward_eval_jitted(variables, xs)
...
print(f"\t\txs_rec shape: {xs_rec.shape}")
...
((zs, neg_pmi), xs_rec), _ = forward(variables, xs, key)

print(zs.shape)
print(neg_pmi.shape)

loss = flo_loss_jitted(zs, neg_pmi)
...
def train_step(state, batch):
    def loss_fn(params):
        # Apply the model
        ((zs, neg_pmi), xs_rec), mutable_updates = forward(
            {
                "params": params,
        ...
        # Compute FLO loss
        flo_loss_val = flo_loss(zs, neg_pmi)
        # Compute reconstruction loss
        rec_loss_val = ((xs_rec - batch["x"]) ** 2).mean()
        ...
        loss = alpha * rec_loss_val + (1 - alpha) * flo_loss_val  # + weight_loss_val
        return loss, {
...
def eval_step(state, batch):
    def loss_fn(variables):
        # Apply the model
        ((zs, neg_pmi), xs_rec), mutable_updates = forward_eval(
            variables,
            batch["x"],
        )
        # Compute FLO loss
