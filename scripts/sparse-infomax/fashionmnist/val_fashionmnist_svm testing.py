...
BINARIZE_K = None # maximum number of non-zero elements to keep, if BINARIZE is True

# remember to change the pooling function in model definition, if using global pool model
default_model = "dense_weights_InfoNCE" #"vgg_sigmoid_and"  # "vgg_sbdr_5softmax/1"  #
default_number = "1"  # "1"  #
...
w_vectors = variables["params"]["w"].copy().T

w_imgs = w_vectors.reshape((w_vectors.shape[0], 28, 28))

# order in terms of unit activity, most active unit first
idx_sort = zs.mean(axis=0).argsort()
w_imgs = w_imgs[idx_sort]

print("\nPlot the learned weights")

# make three rows, one for most activ units, one for mid, one for least active
n_rows = 3
n_cols = 8
half_idx = w_imgs.shape[0] // 2
fig = make_subplots(
    rows=3,
    cols=8,
...
for i in range(n_cols):
    # take the units
    w_most_img = w_imgs[i]
    w_mid_img = w_imgs[half_idx - n_cols//2 +1]
    w_least_img = w_imgs[-n_cols + i]

    # add plots
    fig.add_trace(
        go.Heatmap(z=w_most_img,),
        row=1, col=i+1
    )

    fig.add_trace(
        go.Heatmap(z=w_mid_img,),
        row=2, col=i+1
    )

    fig.add_trace(
        go.Heatmap(z=w_least_img,),
        row=3, col=i+1
    )
