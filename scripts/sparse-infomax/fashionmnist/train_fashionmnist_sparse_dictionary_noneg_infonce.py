...
transform = tv_transforms.Compose(
    [
        ...
        # # # For a non-negative model, input should stay positive
        # # normalize
...
# Create a mask selective of only the dictionary D, we will use it to keep 
# it non-negative
params_mask = unfreeze(jax.tree_util.tree_map(lambda x: False, variables["params"]))
params_mask["D"]["kernel"] = True

# create a chain, we want to clip the dictionary to be non-negative
...
optimizer = optax.chain(
    ...
    # clip dictionary kernel to be non-negative
    optax.masked(optax.keep_params_nonnegative(), params_mask),
)
...
def loss_fn_gen(state, batch, forward_fn):
    def loss_fn(params):
        ...
        z1_loss_val, _ = encoder_infonce(z1, eps=0.05)
        z2_loss_val, z_aux = encoder_infonce(z2, eps=0.05)
        z_loss_val = 0.5 * (z1_loss_val + z2_loss_val)
        # # Reconstruction loss
        x1hat_loss_val, _ = mse(x1_hat, x1_target)
        x2hat_loss_val, xhat_aux = mse(x2_hat, x2_target)
        x_loss_val = 0.5 * (x1hat_loss_val + x2hat_loss_val)
        # Classification loss
        class1_loss_val, _ = classification_loss(logits1, batch["label"])
        class2_loss_val, class_aux = classification_loss(logits2, batch["label"])
        class_loss_val = class1_loss_val + class2_loss_val
        ...
        loss_val = loss_val + (
            +  lam * x_loss_val
            + (1 - lam) * z_loss_val
            + class_loss_val # if there is stop gradient no gradient will flow to z
        )
        ...
        for k, v in xhat_aux.items():
            metrics.update({f"aux_losses/x_{k}": v})
        for k, v in class_aux.items():
            metrics.update({f"aux_losses/class_{k}": v})
...
def train_step(state, batch):
    ...
    # update weights
    updates, opt_state = optimizer.update(grads, state["opt_state"], params=state["variables"]["params"])
...
def compute_activation_imgs(outs, params):
    ...
    # # Add some atoms from the dictionary
    D = params["D"]["kernel"]
    ...
    ds = D[:24]
    # convert to square image
    ds = activation_to_img(ds, normalize=False)
    # add to dictionary for tensorboard
    img_dict.update(
        {f"dictionary/{img_i}": img for img_i, img in enumerate(ds)}
    )
