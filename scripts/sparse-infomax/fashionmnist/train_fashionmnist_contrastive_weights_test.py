""" Train using a contrastive loss on the weights of a single layer
i.e., not on the output.
We assume a single and the loss is a bit "custom", for quick testing

> Note, we need positive inputs and positive weights to have a properly defined loss.
> I manually updated also the initialization of the weights, we need positive values
...
default_model = "dense_weights_InfoNCE"
default_number = "1"
default_cuda = "1"
...
loss = flo_loss_jitted(outs_1, outs_2, variables["params"])
...
def train_step(state, batch):
    def loss_fn(params):
        ...
        sparsity = (outs_1["z"].mean() + outs_2["z"].mean()) / 2
