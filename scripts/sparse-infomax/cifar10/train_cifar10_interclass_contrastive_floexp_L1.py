...
print(xs_1.shape, labels.shape)
print(xs_2.shape, labels.shape)
...
def flo_loss(
    ...
    # # Compute mask so that we contrast only between different classes
    labels_cat = np.argmax(labels, axis=-1)
    label_mask = (labels_cat[..., None] != labels_cat[..., None, :]).astype(outs_1["z"].dtype)
    p_ij_ctx_1_masked = p_ij_ctx_1 * label_mask
    p_ij_ctx_2_masked = p_ij_ctx_2 * label_mask

    # Use the opposite of the mask to compute averages for p_ii, instead of self-only
    p_ii_ctx_masked = (p_ij_ctx_1 * (1 - label_mask)).mean(axis=-1) + (p_ij_ctx_2 * (1 - label_mask)).mean(axis=-1)
    p_ii_ctx_masked = p_ii_ctx_masked / 2

    # compute FLO estimator
    flo_loss_1 = -sbdr.flo_original(u_ii_ctx_1, p_ii_ctx, p_ij_ctx_1_masked, eps=eps, mask_ij=label_mask)
    flo_loss_2 = -sbdr.flo_original(u_ii_ctx_2, p_ii_ctx, p_ij_ctx_2_masked, eps=eps, mask_ij=label_mask)
...
loss = flo_loss_jitted(outs_1, outs_2, labels)
...
def train_step(state, batch):
    def loss_fn(params):
        ...
        # Compute FLO loss
        flo_loss_val = flo_loss(outs_1, outs_2, labels=batch["labels"])
