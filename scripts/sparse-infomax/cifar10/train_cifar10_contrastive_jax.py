...
"""---------------------"""
""" Data Augmentation (to generate positive samples)"""
"""---------------------"""


def augment(key, x):
    key_flip, key_scale, key_translation, key_noise, key_offset = jax.random.split(
        key, 5
    )
    # # FLIP
    if model_config["training"]["augmentation"]["flip"]["bool"]:
        # random flip of images on width dimension for data augmentation
        mask = jax.random.bernoulli(
            key_flip,
            p=model_config["training"]["augmentation"]["flip"]["probability"],
            shape=(*x.shape[:-3], 1, 1, 1),
        )
        # flip on second-to-last last axis, (H,W,C) format is assumed
        x = mask * x + (1 - mask) * np.flip(x, axis=-2)

    # # SCALE AND TRANSLATE
    if model_config["training"]["augmentation"]["crop"]["bool"]:
        # random scaling of images. NOTE: same scale factor on both dimension, keep the aspect ratio
        scale = jax.random.uniform(
            key_scale,
            minval=model_config["training"]["augmentation"]["crop"]["scale"][0],
            maxval=model_config["training"]["augmentation"]["crop"]["scale"][1],
        )
        ...
        translation = jax.random.uniform(
            key_translation,
            shape=(2,),
            minval=model_config["training"]["augmentation"]["crop"]["translation"][0],
            maxval=model_config["training"]["augmentation"]["crop"]["translation"][1],
        )
        # scale and translate
    ...
    # # GAUSSIAN NOISE
    if model_config["training"]["augmentation"]["noise"]["bool"]:
        # add random noise to images
        noise = model_config["training"]["augmentation"]["noise"][
            "scale"
        ] * jax.random.normal(key_noise, shape=x.shape)
        x = x + noise

    # # # CHANNEL OFFSET
    if model_config["training"]["augmentation"]["channel_offset"]["bool"]:
        # add a random offset to each channel
        offset = model_config["training"]["augmentation"]["channel_offset"][
            "scale"
        ] * jax.random.normal(
            key_offset,
            shape=(
...
# xs_augmented = augment(jax.random.key(int(time())), xs)


# def untransform(xs):
#     return (xs * np.array([0.2470, 0.2435, 0.2616])) + np.array(
...
# # converted to BGR
# for i, (img, img_augmented) in enumerate(zip(xs, xs_augmented)):
#     img = untransform(img)
#     img_augmented = untransform(img_augmented)
#     # clip in range 0-1
#     img = np.clip(img, 0, 1)
#     img_augmented = np.clip(img_augmented, 0, 1)
#     # resize
#     img = jax.image.resize(img, (128, 128, 3), "bilinear")
#     img_augmented = jax.image.resize(img_augmented, (128, 128, 3), "bilinear")
#     # convert to BGR
#     img = np.flip(img, axis=-1)
#     img_augmented = np.flip(img_augmented, axis=-1)
#     # convert to 0-255
#     img = (img * 255).astype(np.uint8)
#     img_augmented = (img_augmented * 255).astype(np.uint8)
#     # convert to original NumPy array
#     img = onp.array(img)
#     img_augmented = onp.array(img_augmented)
#     # show image
...
def train_step(state, batch):
    def loss_fn(params):
        ...
        outs_aug, mutable_updates_aug = forward(
            {
                "params": params,
                # # BATCH_NORM - change here
                # "batch_stats": state["variables"]["batch_stats"],
