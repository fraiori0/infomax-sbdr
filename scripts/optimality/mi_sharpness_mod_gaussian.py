...
if __name__ == "__main__":
    ...
    sim_fns = {
        ...
        "and": {
            "self": jit(sbdr.expected_and),
            "cross": jit(sbdr.expected_and),
        },
    ...
    for n_nonzero in tqdm(GAMMA_AVERAGE_NON_ZERO, disable=disable_tqdm):
        ...
        for i, mu in enumerate(tqdm(MU, leave=False, disable=disable_tqdm)):
            ...
            for seed in tqdm(SEEDS, leave=False, disable=disable_tqdm):
                ...
                key, _ = jax.random.split(key)
                activation_values = mu + SIGMA * jax.random.normal(
                    key, shape=zs_mask.shape
                )
                activation_values = np.clip(activation_values, 0, 1)
