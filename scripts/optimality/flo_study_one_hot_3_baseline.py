...
if __name__ == "__main__":
        ...
        for i, beta_params in enumerate(
            ...
            for mean_noise_units in tqdm(
                ...
                if i < (len(beta_params) - 1):
                    ...
                    zs = jax.random.beta(
                        key, alpha, beta, shape=(N_SAMPLES, N_TOT_FEATURES)
                    )
                else:
                    zs = np.ones((N_SAMPLES, N_TOT_FEATURES))

                # Generate mask with fixed amount of non-zero features
                if n_nonzero_features == 0:
                    zs_mask_fixed = np.zeros((N_SAMPLES, N_TOT_FEATURES), dtype=bool)
                else:
                    key, _ = jax.random.split(key)
        ...
        for j, _ in enumerate(ALPHAS_BETAS):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    legendgroup=f"{nonzero_feature}",  # f"{mean}",  #
    ...
    fig.update_layout(
        title=f"MI vs Added Noise - {N_TOT_FEATURES} Features<br>Similarity: {SIMILARITY}<br>N Samples: {N_SAMPLES}<br>Negative Noise: {NOISE_NEG}",
