...
if __name__ == "__main__":
        ...
        for i, beta_params in enumerate(
            ...
            for mean_noise_units in tqdm(
                ...
                if i < (len(beta_params) - 1):
                    ...
                    zs = jax.random.beta(
                        key, alpha, beta, shape=(N_RESAMPLE, N_SAMPLES, N_TOT_FEATURES)
                    )
                else:
                    zs = np.ones((N_RESAMPLE, N_SAMPLES, N_TOT_FEATURES))

                # Generate mask with fixed amount of non-zero features
                if n_nonzero_features == 0:
                    zs_mask_fixed = np.zeros(
                        (N_RESAMPLE, N_SAMPLES, N_TOT_FEATURES), dtype=bool
                    )
                else:
                    key, _ = jax.random.split(key)
                    zs_mask_fixed = gen_ys_k_active(
                        key, n_nonzero_features, N_RESAMPLE * N_SAMPLES, N_TOT_FEATURES
                    )
                zs_mask_fixed = zs_mask_fixed.reshape(
                    N_RESAMPLE, N_SAMPLES, N_TOT_FEATURES
                )
                ...
                zs_mask_noise_pos = jax.random.bernoulli(
                    ...
                    shape=(N_RESAMPLE, N_SAMPLES, N_TOT_FEATURES),
                )
                ...
                # # Generate masks considering a fixed amount of masking units
                # if mean_noise_units == 0.0:
                #     zs_mask_noise_fixed_pos = np.zeros(
                #         (N_SAMPLES, N_TOT_FEATURES), dtype=bool
                #     )
                #     zs_mask_noise_fixed_neg = np.ones(
                #         (N_SAMPLES, N_TOT_FEATURES), dtype=bool
                #     )
                # else:
                #     key, _ = jax.random.split(key)
                #     zs_mask_noise_fixed_pos = gen_ys_k_active(
                #         key, mean_noise_units, N_SAMPLES, N_TOT_FEATURES
                #     )
                #     # shift by one to get the negative mask
                #     zs_mask_noise_fixed_neg = np.logical_not(
                #         np.roll(zs_mask_noise_fixed_pos, 1, axis=-1)
                #     )
    ...
    fig.update_layout(
        title=f"MI distribution - {N_TOT_FEATURES} Features<br>Similarity: {SIMILARITY}<br>N Samples: {N_SAMPLES}<br>Negative Noise: {NOISE_NEG}",
    ...
    if SAVE_FIGS:
        fig_name = f"mi_vs_noise_{SIMILARITY}_{N_TOT_FEATURES}_features_{N_SAMPLES}_samples_{NOISE_NEG}_noise_neg"
