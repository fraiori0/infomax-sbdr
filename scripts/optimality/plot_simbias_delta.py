...
if __name__ == "__main__":
    ...
    # MIs = MIs[SIM_KEY]  # shape (n_nonzero, eps_sim, seeds)
    ...
    for k, sim_k in enumerate(MIs.keys()):
        for i, e_sim in enumerate(eps_sim):
            mi = MIs[sim_k][:, i, :]  # mi : shape (n_nonzero, seeds)

            hue = i / len(eps_sim)
            lightness = 0.5 # + 0.25 * (j / len(n_nonzero))
            saturation = 0.7
