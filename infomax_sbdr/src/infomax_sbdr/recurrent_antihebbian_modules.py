...
""" -------------------- """
""" Recurrent version of the Anti-Hebbian Modules """
""" -------------------- """


class AntiHebbianRecurrentBase(nn.Module):
    """Base class for a recurrent Anti-Hebbian Layer, with forward (input) and lateral weights."""
    ...
    def forward_scan(self, x_seq, z0):
        """ Iteratively apply the network to a sequence of inputs """
        raise NotImplementedError

    def compute_dmu(self, params, z_prev, z_curr, momentum):
        # input z of shape (*batch, features)

        # matrix of coactivations
        zi_zj = z_curr[..., :, None] * z_curr[..., None, :]
        ...
        dmu = dmu.reshape(-1, z_curr.shape[-1], z_curr.shape[-1]).mean(axis=0)

        return dmu

    def compute_dw_f(self, params, x, z_prev, y, z):
        # x and z as returned from the __call__ method

        kernel = params["params"]["w_f"]["kernel"]
        mu_self = np.diag(params["params"]["mu"])

        xz = np.concatenate((x, z_prev), axis=-1)

        xi_zj = (xz[..., :, None] - kernel) * z[..., None, :]
    ...
    def compute_dw_l(self, params, x, z_prev, y, z):
        # x and z as returned from the __call__ method
    ...
    def compute_dparams(self, params, x, z_prev, y, z, momentum=None):
        # x and z as returned from the __call__ method
        dparams = {
            "params": {
                "mu": None,
        ...
        if momentum is None:
            dmu = self.compute_dmu(params, z_prev, z, self.momentum)
        else:
            dmu = self.compute_dmu(params, z_prev, z, momentum)
        ...
        dw_f = self.compute_dw_f(tmp_params, x, z_prev, y, z)
        dw_l = self.compute_dw_l(tmp_params, x, z_prev, y, z)
    ...
    def update_params(self, params, x, z_prev, y, z, lr: float = 0.1, momentum=None):

        dparams = self.compute_dparams(params, x, z_prev, y, z, momentum)

        params = self.apply_dparams(params, dparams, lr)

        return params, dparams
    
    def generate_initial_state(self, key, x, p=None, a_min=0.0, a_max=1.0):
        if p is None:
            z0 = np.zeros((*x.shape[:-1], self.n_features))
        else:
            z0 = jax.random.bernoulli(key, p=p, shape=(*x.shape[:-1], self.n_features))
            # scale [0,1] to [a_min, a_max]
            z0 = a_min + (a_max - a_min) * z0
        return z0
...
class AntiHebbianRecurrentModule(AntiHebbianRecurrentBase):
    """ Recurrent module with anti-hebbian learning """
    ...
    def __call__(self, x, z_prev, rngs={'dropout': jax.random.PRNGKey(0)}):
        # input x of shape (*batch, in_features)
        # input z_prev of shape (*batch, features)
        # i.e., already flattened on the feature dimensions, if multiple (like image C, H, W)

        # Use as input the actual input and the previous activation, like in an Elman network.
        xz = np.concatenate((x, z_prev), axis=-1)

        y = self.w_f(xz) > 0

        if self.use_dropout:
            z = self.w_l(self.dropout(y.astype(x.dtype), rng=rngs["dropout"])) > 0
        else:
            z = self.w_l(y) > 0
    ...
    def forward_scan(self, x_seq, z0, key=jax.random.PRNGKey(0)):
        """Iteratively apply the network to a sequence of inputs"""
        ...
        def f_scan(carry, input):
            ...
            if self.use_dropout:
                out = self(x, z_prev, rngs={"dropout": rng_key})
            else:
                out = self(x, z_prev)

            gamma = 0.2
            z_prev_next = (1-gamma)*z_prev + gamma*out["z"]

            return z_prev_next, out

        # note, we neeed to place the time axis as the first.
        # We assume input to this function are always given with shape (*batch_dims, time, in_features)
        x_seq = np.moveaxis(x_seq, -2, 0)

        # generate keys for the whole scan
        rng_keys = jax.random.split(key, x_seq.shape[0])

        # Perform the scan
        _, out_seq = jax.lax.scan(f_scan, z0, (rng_keys, x_seq))

        # Put back time axis
        out_seq = jax.tree.map(lambda x: np.moveaxis(x, 0, -2), out_seq)
